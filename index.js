...
function findIndexByProperty(arr, name) {
  ...
  const FindIndex = arr.findIndex((obj) => obj.name === name);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const sliceArray = arr.slice(startIdx);
  // За допомогою методу join() конвертуємо підмасив в рядок.
  const StringArray = sliceArray.join(" ");
  // Повертаємо рядок.
  return StringArray;
}
...
function getArrayKeys(arr) {
  ...
  const arrayKeys = Array.from(keyIterator);
...
function getPositiveArrayValues(arr) {
  ...
  const convert = Array.from(Values);
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  const Filter = convert.filter((number) => number > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const Double = arr.map((number) => number * startIdx);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastNElements.reduce((acc, num) => acc + num, 0);
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((element) => {
    if (elementCounts[element]) {
      return true;
    }
    elementCounts[element] = arr.indexOf(element);
    return false;
  });
